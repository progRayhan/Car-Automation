              ...
              <div className="flex flex-col -mx-4 md:flex-row md:items-center md:mx-8">
                {navItem.map((link) => (
                  <Link href={link.link} key={link.link} passHref>
                    <button className="px-2 py-1 uppercase mx-2 mt-2 text-sm font-medium text-gray-700 transition-colors duration-200 transform rounded-md md:mt-0 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-700">
                      {link.level}
                    </button>
                  </Link>
                ))}
